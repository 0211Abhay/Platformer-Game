class UIManager
{
    displayLivesCount(player)
    {
        this.livesCountUI = add([
            text("",{
                font:"Round",size:50}),fixed(),pos(70,10)
        ])
        this.livesCountUI.add([
            sprite("star-icon"),
            pos(-60,-5),
    ...
    {
        this.coinCountUI = add([text("",{
            font:"Round",
            size:50
        }),{
        fullCoinCount:get("coin",{recursive : true}).length},fixed(),pos(70,70)])
        this.coinCountUI.add([sprite("coin-icon"),
        pos(-60,0),scale(3),fixed()])
    }
    ...
    displayMainMenu(){
        ...
        this.displayBlinkingUIMessage("Press [ ENTER ] to Start Game",vec2(center().x,center().y+100))
        onKeyPress("enter",()=>{
            play("confirm-ui",{
                speed:1.5
            })
    ...
    displayControlsMenu(){
        ...
        controlPrompts.add([
          ...
          this.displayBlinkingUIMessage("Press [ ENTER ] to Start Game",vec2(center().x,center().y+300))
          onKeyPress("enter",()=>{
            play("confirm-ui",{
                speed:1.5
            })
